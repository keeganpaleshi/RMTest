...
def emg_pdf_E(E, mu, sigma, tau):
    ...
    When tau is very small, degenerates to Gaussian.
    """
    E = np.asarray(E, dtype=float)
    if sigma <= 0:
        return np.zeros_like(E, dtype=float)
    if tau <= 0 or tau < 1e-10:
        # Degenerate to Gaussian when tau is zero or very small
...
def emg_cdf_E(E, mu, sigma, tau):
    ...
    if tau <= 0 or tau < 1e-10:
        # Degenerate to Gaussian when tau is zero or very small
        return gaussian_cdf_E(E, mu, sigma)
    K = tau / sigma
