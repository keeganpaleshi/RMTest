...
- `bkg_mode` – `"auto"` estimates the linear continuum from the spectrum
  while `"manual"` uses the `S_bkg_prior` / `beta0_prior` / `beta1_prior` values.
- `S_bkg_prior`, `beta0_prior`, `beta1_prior` – `[mean, sigma]` priors for the
  background amplitude and shape parameters.
