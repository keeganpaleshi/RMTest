...
concentration simply convert the fitted rate to Bq/m^3 using `utils.cps_to_bq`.
For example:
...
and used only as the starting sigma for the Po-214 peak fit; it never replaces the
fitted sigma_E that is written to summary.json.
...
radon concentration in Bq/L (converted internally to Bq/m³) used for the
equivalent air plot.  The command-line option `--ambient-concentration`
overrides this value.  The default configuration sets this key to `null`.  The template
...
scaled by 1000 to Bq/m³ and override any constant value when calling
`plot_equivalent_air`.
...
- `unbinned_likelihood` - when `true` use an extended unbinned likelihood
  instead of the default chi^2 fit to histogrammed data.
...
physical half-lives of 1.64x10^-4 s, 186 s and 1.1956x10^7 s respectively.
