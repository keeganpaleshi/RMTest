...
def emg_left(x, mu, sigma, tau):
    ...
    We provide two interchangeable implementations: the legacy
    :func:`scipy.stats.exponnorm.pdf` helper and a stabilized wrapper.
    The stabilized version primarily adds NaN/Inf cleaning and a
    small-``tau`` Gaussian fallback; it does **not** change the
    underlying EMG lineshape.
    ...
    if USE_STABLE_EMG:
        # Use stabilized wrapper around scipy.stats.exponnorm
