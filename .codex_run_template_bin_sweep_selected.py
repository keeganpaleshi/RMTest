...
BASE_CONFIG = WORKSPACE / ".codex_run_merged_output_1h_template_boundsummary.yaml"
INPUT_CSV = WORKSPACE / "merged_output.csv"
HOURS = [24, 18, 14, 10, 8, 6, 4, 2, 1]
...
def main() -> int:
    ...
    sweep_root = WORKSPACE / f".codex_run_template_bin_sweep_selected_{session_id}"
    ...
    for hour in HOURS:
        ...
        if template_plot_dir is not None and template_plot_dir.exists():
            base_plot_count = len(
                [
                    p
                    for p in template_plot_dir.glob("template_fit_bin_*.png")
