...
def fit_hierarchical_runs(run_results, draws=2000, tune=1000, chains=2, random_seed=42):
    ...
    hl_err = np.array(
        [max(r.get("dhalf_life", np.nan), 1e-6) for r in run_results], dtype=float
    )
    ...
    if use_slope:
        ...
        slope_err = np.array(
            [max(r.get("dslope", np.nan), 1e-6) for r in run_results], dtype=float
        )
    ...
    if use_intercept:
        ...
        int_err = np.array(
            [max(r.get("dintercept", np.nan), 1e-6) for r in run_results], dtype=float
        )
    ...
    with pm.Model() as model:
        ...
        if use_slope:
            mu_a = pm.Normal(
                "mu_a", mu=slope_obs.mean(), sigma=10 * slope_obs.std() + 1e-6
            )
    ...
    out = {
        "half_life": {
            ...
            "hdi": [
                float(summ.loc["mu_hl", "hdi_2.5%"]),
                float(summ.loc["mu_hl", "hdi_97.5%"]),
            ],
        }
    }
    ...
    if use_slope:
        out["slope"] = {
            ...
            "hdi": [
                float(summ.loc["mu_a", "hdi_2.5%"]),
                float(summ.loc["mu_a", "hdi_97.5%"]),
            ],
        }
    if use_intercept:
        out["intercept"] = {
            ...
            "hdi": [
                float(summ.loc["mu_c", "hdi_2.5%"]),
                float(summ.loc["mu_c", "hdi_97.5%"]),
            ],
        }
    return out
