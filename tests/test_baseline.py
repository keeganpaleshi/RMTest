...
import radon.baseline as radon_baseline
...
def test_baseline_subtraction_uses_fitted_efficiency(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(baseline_utils, "subtract_baseline_counts", lambda *a, **k: (-5.0, 0.2))
    monkeypatch.setattr(radon_baseline, "subtract_baseline_counts", lambda *a, **k: (-5.0, 0.2))
    monkeypatch.setattr(analyze, "subtract_baseline_counts", lambda *a, **k: (-5.0, 0.2))
...
def test_allow_negative_baseline_preserves_corrected_rate(tmp_path, monkeypatch):
    cfg = {
        "pipeline": {"log_level": "INFO"},
        "baseline": {"range": [0, 10], "monitor_volume_l": 605.0, "sample_volume_l": 0.0},
        "calibration": {},
    ...
    monkeypatch.setattr(baseline_utils, "subtract_baseline_rate", fake_subtract)
    monkeypatch.setattr(analyze, "subtract_baseline_rate", fake_subtract)
    monkeypatch.setattr(radon_activity, "compute_radon_activity", lambda *a, **k: (0.1, 0.01))
    monkeypatch.setattr(
        radon_activity,
        "compute_total_radon",
        lambda *a, **k: (0.1, 0.01, 0.1, 0.01),
    )
    ...
    tf_entry = summary["time_fit"].get("Po214", {})
    assert tf_entry.get("E_corrected") == pytest.approx(-5.0)
