...
def test_negative_baseline_preserved_when_allowed(tmp_path, monkeypatch):
    cfg = {
        "pipeline": {"log_level": "INFO"},
        "baseline": {"range": [0, 10], "monitor_volume_l": 605.0, "sample_volume_l": 0.0},
        "calibration": {},
    ...
    monkeypatch.setattr(radon_activity, "compute_radon_activity", lambda *a, **k: (1.0, 0.1))
    monkeypatch.setattr(
        radon_activity,
        "compute_total_radon",
        lambda *a, **k: (1.0, 0.1, 1.0, 0.1),
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: {"Rn_activity_Bq": 1.0, "stat_unc_Bq": 0.1},
    )
    ...
    summary = captured["summary"]
    corr_rate = summary["baseline"]["corrected_rate_Bq"]["Po214"]
    corr_sigma = summary["baseline"]["corrected_sigma_Bq"]["Po214"]

    assert subtract_calls, "subtract_baseline_rate was not invoked"
    assert corr_rate == pytest.approx(-5.0)
    assert corr_sigma == pytest.approx(0.5)
