...
def test_load_events(tmp_path, caplog):
    ...
    expected_ts = pd.Series([parse_datetime(t) for t in (1000, 1005, 1010)])
    assert loaded["timestamp"].reset_index(drop=True).equals(expected_ts)
...
def test_load_events_drop_bad_rows(tmp_path, caplog):
    ...
    assert loaded["timestamp"].dtype == "datetime64[ns, UTC]"
    expected_ts = pd.Series([parse_datetime(t) for t in (1000, 1005, 1020)])
    assert loaded["timestamp"].reset_index(drop=True).equals(expected_ts)
