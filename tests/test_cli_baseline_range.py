...
def test_time_fit_background_mode_floated(tmp_path, monkeypatch):
    ...
    trend_sample_volumes: list[float | None] = []
    ...
    def fake_plot_radon_trend(*args, **kwargs):
        out_png = Path(args[2])
        out_png.parent.mkdir(parents=True, exist_ok=True)
        out_png.touch()
        trend_sample_volumes.append(kwargs.get("sample_volume_l"))
        return None

    monkeypatch.setattr(analyze, "plot_radon_trend", fake_plot_radon_trend, raising=False)
    ...
    assert total_calls
    assert trend_sample_volumes
    assert activity_calls[-1] == expected_mode
    assert total_calls[-1] == expected_mode
    assert trend_sample_volumes[-1] == pytest.approx(0.0)


def test_radon_inference_does_not_overwrite_radon_trend(tmp_path, monkeypatch):
    cfg = {
        "pipeline": {"log_level": "INFO"},
        "baseline": {"monitor_volume_l": 605.0, "sample_volume_l": 5.0},
        "calibration": {},
        "spectral_fit": {"do_spectral_fit": False, "expected_peaks": {"Po210": 0}},
        "time_fit": {
            "do_time_fit": True,
        ...
        "radon_inference": {
            ...
            "detection_efficiency": {"Po214": 1.0},
        },
        "systematics": {"enable": False},
        "plotting": {"plot_save_formats": ["png"]},
    }
    ...
    monkeypatch.setattr(analyze, "plot_total_radon", lambda *a, **k: Path(a[2]).touch())
    monkeypatch.setattr(analyze, "plot_rn_inferred_vs_time", lambda *a, **k: None)
    monkeypatch.setattr(analyze, "plot_ambient_rn_vs_time", lambda *a, **k: None)
    monkeypatch.setattr(analyze, "plot_volume_equiv_vs_time", lambda *a, **k: None)
    ...
    def fake_plot_radon_trend(*args, **kwargs):
        ...
        out_png.touch()
        trend_calls.append(
            {"times": list(args[0]), "sample_volume_l": kwargs.get("sample_volume_l")}
        )
        return None
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: {
            ...
            "ambient_rn": [
                {"t": 0.5, "rn_bq_per_m3": 80.0},
                {"t": 1.5, "rn_bq_per_m3": 80.0},
            ],
            "volume_equiv": [
                {"t": 0.5, "v_m3": 0.01},
                {"t": 1.5, "v_m3": 0.02},
    ...
    monkeypatch.setattr(
        analyze,
        "write_summary",
        lambda out_dir, summary, timestamp=None: str(Path(out_dir) / (timestamp or "x")),
    )
    monkeypatch.setattr(analyze, "copy_config", lambda *a, **k: None)
    ...
    assert len(trend_calls) == 2
    assert all(call["sample_volume_l"] == pytest.approx(5.0) for call in trend_calls)
    assert not any(call["times"] == [0.5, 1.5, 2.5] for call in trend_calls)
