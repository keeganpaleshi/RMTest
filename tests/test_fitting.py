...
from fitting import fit_time_series, fit_spectrum, FitResult, _TAU_MIN, softplus
import analyze
...
def test_fit_spectrum_background_only_irregular_edges():
    ...
    bkg_per_mev = softplus(result.params["S_bkg"]) / width
    assert np.isclose(bkg_per_mev, 10.0, atol=0.1)
...
def test_spectrum_tail_amplitude_stability():
    ...
    tol = 0.1  # allow 10 % window due to background coupling
