...
Purpose: Wrapper around ``scipy.stats.exponnorm`` with additional stability hygiene
Author: RMTest Enhancement Module

This module provides a thin wrapper around the Exponentially Modified Gaussian
(EMG) distribution for use in radon alpha spectroscopy peak fitting.

Key Features:
- Adds safeguards against overflow/underflow when evaluating the PDF
- Falls back to a Gaussian for very small tau values (at the configured minimum)
- Cleans NaN/Inf results to keep downstream code robust
...
class StableEMG:
    """Stabilized wrapper around the Exponentially Modified Gaussian PDF."""
...
def emg_left_stable(x, mu, sigma, tau, amplitude: float = 1.0, use_log_scale: bool = False):
    ...
    while adding light-weight numerical hygiene around ``scipy.stats.exponnorm``.
