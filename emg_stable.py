...
Purpose: Stabilized wrapper around ``scipy.stats.exponnorm`` for the
Exponentially Modified Gaussian distribution.
Author: RMTest Enhancement Module

This module keeps the standard ``scipy.stats.exponnorm`` lineshape but adds
practical safeguards (NaN/Inf cleaning, small-``tau`` fallback, optional
parameter validation) for radon alpha spectroscopy peak fitting.

Key Features:
- Delegates to ``scipy.stats.exponnorm`` while adding numerical guard rails
...
class StableEMG:
    """Wrapper around ``scipy.stats.exponnorm`` with additional safeguards."""
    ...
    def pdf(self,
        ...
        """
        Calculate EMG probability density function with additional safeguards.

        This implementation uses ``scipy.stats.exponnorm`` under the hood but
        adds bounds checks, NaN/Inf cleaning, and a small-``tau`` fallback.
...
def emg_left_stable(x, mu, sigma, tau, amplitude: float = 1.0, use_log_scale: bool = False):
    ...
    This function wraps the existing EMG implementation but adds NaN/Inf cleaning
    and a Gaussian fallback for extremely small ``tau`` values while preserving
    the ``scipy.stats.exponnorm`` lineshape.
