...
def fit_spectrum(
    ...
    *,
    ...
    fit_likelihood = "unbinned" if unbinned else "binned_poisson"
    ...
    if not unbinned:
        ...
        if not m.valid:
            ...
            return FitResult(
                out, cov, int(ndf), param_index, counts=int(n_events), likelihood=fit_likelihood
            )
    ...
    param_index = {name: i for i, name in enumerate(param_order)}
    return FitResult(
        out, pcov, int(ndf), param_index, counts=int(n_events), likelihood=fit_likelihood
    )
