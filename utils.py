...
def parse_datetime(value):
    """Parse an ISO-8601 string or numeric epoch to a ``pandas.Timestamp``.

    The function accepts strings like ``"2023-09-28T13:45:00-04:00"`` or numeric
    Unix timestamps (``int``, ``float`` or numeric ``str``). Any parsed time
    lacking a timezone is interpreted as UTC. On success a timezone-aware
    ``pandas.Timestamp`` in UTC is returned. ``ValueError`` is raised if the
