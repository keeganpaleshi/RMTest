...
def parse_datetime(value):
    """Parse an ISO-8601 string or numeric epoch value to a UTC
    ``pandas.Timestamp``.
    ...
    timezone aware ``pandas.Timestamp`` in UTC (nanosecond resolution) is
    returned. ``ValueError`` is raised if the input cannot be parsed.
    """
