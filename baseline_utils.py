...
def _to_datetime64(col: pd.Series | pd.Index) -> pd.Series | pd.Index:
...
def subtract_baseline_dataframe(
    ...
    t0 = pd.to_datetime(parse_datetime(t_base0), utc=True)
    t1 = pd.to_datetime(parse_datetime(t_base1), utc=True)
