...
def _model_counts_per_bin(edges, fit_vals):
    """Return expected counts per bin for the spectral model.
    ...
    edges : array-like
        Bin edges in MeV.
    fit_vals : dict
        Fit parameter dictionary containing background and peak terms.
    """
...
def plot_spectrum(
    ...
    if fit_vals:
        model_counts = _model_counts_per_bin(edges, fit_vals)
        palette_name = str(config.get("palette", "default")) if config else "default"
        palette = COLOR_SCHEMES.get(palette_name, COLOR_SCHEMES["default"])
